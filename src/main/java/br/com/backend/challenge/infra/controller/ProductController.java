...
public class ProductController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductResponseDTO> updateProduct(@PathVariable("id") Integer id, @RequestBody @Valid ProductRequestDTO productDTO) {
        Product product = ProductDTOMapper.toProduct(productDTO);
        Product createdProduct = updateProductUseCase.updateProduct(id, product);
        return ResponseEntity.status(HttpStatus.OK).body(ProductDTOMapper.toResponse(createdProduct));
    }
